...
def key_asterisk(prompt):
    """
    Secure input function that hides characters with asterisks
    and supports paste operations. Works on both Windows and Unix-like systems.
    ...
    if os.name == 'nt':  # Windows
        ...
        while True:
            if msvcrt.kbhit():  # Check if key is pressed
                ...
                elif char == b'\xe0':  # Special key prefix on Windows
                    msvcrt.getch()  # Consume the next character
...
def key_asterisk_all(prompt):
    """
    Enhanced version that specifically handles paste operations better.
    """
    if os.name == 'nt':  # Windows
        import msvcrt
        import time
        ...
        while True:
            if msvcrt.kbhit():
                ...
                # Check for rapid input (likely paste)
                if current_time - last_input_time < 0.01:
                    # Handle as potential paste
                    pasted_chars = char.decode('utf-8', errors='ignore')

                    # Read any additional characters that come quickly
                    while msvcrt.kbhit():
                        next_char = msvcrt.getch()
                        if 32 <= ord(next_char) <= 126:
                            pasted_chars += next_char.decode('utf-8', errors='ignore')
                        elif next_char == b'\r':
                            password += pasted_chars
                            print('*' * len(pasted_chars), end='', flush=True)
                            print()
                            return password
...
def get_credentials():
    ...
    if choice == "1":
        ...
        api_key = key_asterisk_all(f"{pystyle.Colors.reset}API Key:  {pystyle.Colors.red}") #api_key = input("API Key: ").strip() #api_key = getpass.getpass("API Key: ").strip()
...
def get_download_options():
    ...
    # Get folder name
    default_folder = tags.replace(" ", "_").replace(":", "-").replace("\\", "-").replace("/", "-").replace("*", "-").replace('"', "'").replace("|", "-").replace("<", "-").replace(">", "-").replace("?", "-")
    folder_name = default_folder
    
    # Ask about zipping
    zip_choice = input(f"{pystyle.Colors.reset}Zip folder after download? (y/N): {pystyle.Colors.green}").strip().lower() in ['y', 'yes']
    
    # Ask about skipping duplicates
...
async def run_cli_scraper(options, username=None, api_key=None):
    ...
    downloaded_count, skipped_count = await start_scraper(
        ...
        total_posts=options['post_count'],
...
async def start_scraper(query_tags, total_posts, thread_limit, download_folder, debug=False, auth=None, tracker=None, duplicate_detector=None, skip_duplicates=True):
    os.makedirs(download_folder, exist_ok=True)
    sem = asyncio.Semaphore(thread_limit)
    downloaded = 0
    skipped_duplicates = 0
    ...
    async with aiohttp.ClientSession(auth=auth) as session:
        while downloaded < total_posts:
            batch = min(320, total_posts - downloaded)
            params = {
                "tags": query_tags,
                "limit": str(batch),
                "page": str(page)
            ...
            for post in posts:
                ...
                if file_url:
                    tasks.append(download_file(
                        sem, file_url, post["id"], session,
                        download_folder, debug, auth, None,
                        tracker, duplicate_detector, skip_duplicates,
                        update_progress
                    ))
