# YiffScraper CLI v1.0.0 - "iOS" Version (API Key visiblity)
...
def get_credentials():
    """Get login credentials from user - Mobile Version"""
    print()
    print()
    print("=== Auth ===")
    print("----------")
    ...
    if choice == "1":
        print()
        print()
        print(f"{pystyle.Colors.yellow}[INFO] API key will be visible on screen for mobile compatibility{pystyle.Colors.reset}")
        print()

        username = input(f"{pystyle.Colors.reset}Username: {pystyle.Colors.red}").strip()
        api_key = input(f"{pystyle.Colors.reset}API Key: {pystyle.Colors.red}").strip()
...
def show_banner():
    ...
    print("   Mobile/iOS Version (Simple)")
